...
prompt_right_segment() {
  local bg fg r_bg r_fg
  [[ -n $1 ]] && bg="%K{$1}" || bg="%k"
  [[ -n $1 ]] && r_fg="%F{$1}" || r_fg="%f"
  [[ -n $2 ]] && fg="%F{$2}" || fg="%f"
  [[ -n $2 ]] && r_bg="%F{$2}" || r_bg="%f"
  if [[ $CURRENT_BG != 'NONE' && $1 != $CURRENT_BG ]]; then
    echo -n " ${r_fg}$RIGHT_SEGMENT_SEPARATOR%{$bg%F{$CURRENT_BG}%}%{$fg%} "
  else
    echo -n "${r_fg}$RIGHT_SEGMENT_SEPARATOR%{$bg%}%{$fg%} "
  fi
  CURRENT_BG=$1
  [[ -n $3 ]] && echo -n $3
}
...
prompt_context() {
  if [[ "$USER" != "$DEFAULT_USER" || -n "$SSH_CLIENT" ]]; then
    #prompt_segment 8 white "%(!.%{%F{yellow}%}.)$USER@%m"
...
prompt_knife() {
	if [[ -L ~/.chef/knife.rb ]]; then
      ...
      prompt_segment red white "${env_name}"
		  ...
		  prompt_segment white black "${env_name}"
	  fi
	fi
}
...
build_line1() {
  ...
  local right=$(
    prompt_right_segment white black
    echo -n "[%T]"
    right_prompt_end
  )
  ...
  local leftcontent=${(S)left//$~invisible}
  local rightcontent=${(S)right//$~invisible}
  local content="${leftcontent}${rightcontent}"
  local prompt_length="${#${(%)${content}}}"
  local pad="${(l,COLUMNS-prompt_length,)}"
